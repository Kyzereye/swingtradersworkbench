 ...
 *   npm run analyze-ma-crossover -- --limit 10
 */
...
async function analyzeOne(sym) {
  ...
  const result = scanMaCrossover(bars);
  if (!result) {
    return { sym, status: "skip", reason: "analyze failed" };
  }
  await upsertMaCrossoverScanRow(sym, result);
...
async function main() {
  ...
  console.log(`MA crossover optimize ${symbols.length} symbol(s)…\n`);
