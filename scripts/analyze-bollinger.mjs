/**
 * Optimize Bollinger squeeze per symbol → system_bollinger_scan.
 *
 *   npm run analyze-bollinger
 *   npm run analyze-bollinger -- --symbol AAPL
 *   npm run analyze-bollinger -- --limit 10
 */

import { scanBollinger } from "../frontend/src/scanBollinger.js";
import { closePool } from "../backend/src/db.js";
import {
  listActiveSymbols,
  loadBarsForSymbol,
  upsertBollingerScanRow,
...
async function analyzeOne(sym) {
  ...
  const result = scanBollinger(bars);
  if (!result) {
    return { sym, status: "skip", reason: "analyze failed" };
  }
  await upsertBollingerScanRow(sym, result);
...
async function main() {
  ...
  if (!PIPELINE_QUIET) {
    console.log(`Bollinger optimize ${symbols.length} symbol(s)…\n`);
  }
    ...
    try {
      ...
      if (r.status === "ok") {
        ...
        if (!PIPELINE_QUIET) {
          ...
          console.log(
            `${sym.padEnd(6)} ${r.optPeriod}/${r.optStdMult} atr${r.optAtrPeriod}x${r.optAtrMult}  RT ${rt}  ${pct}  trades ${r.tradeCount}  ${r.lastSignal}`
          );
        }
      } else {
        skipped++;
