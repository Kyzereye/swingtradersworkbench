/**
 * Optimize MACD per symbol (max 1-share $ P/L, full history)
 * → system_macd_scan (stored totals = last ~2y).
 *
 *   npm run analyze-macd
 *   npm run analyze-macd -- --symbol AAPL
 *   npm run analyze-macd -- --limit 10
 */

import { scanMacd } from "../frontend/src/scanMacd.js";
...
async function analyzeOne(sym) {
  ...
  const result = scanMacd(bars);
  if (!result) {
    return { sym, status: "skip", reason: "analyze failed" };
  }
  await upsertMacdScanRow(sym, result);
...
async function main() {
  ...
  if (!PIPELINE_QUIET) {
    console.log(`MACD optimize ${symbols.length} symbol(s)…\n`);
  }
    ...
    try {
      ...
      if (r.status === "ok") {
        ...
        if (!PIPELINE_QUIET) {
          ...
          console.log(
            `${sym.padEnd(6)} ${r.optFast}/${r.optSlow}/${r.optSignal}  RT ${rt}  ${pct}  trades ${r.tradeCount}  ${r.lastSignal}`
          );
        }
      } else {
        skipped++;
