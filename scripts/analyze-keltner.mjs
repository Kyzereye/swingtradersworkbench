/**
 * Optimize Keltner Channel trend per symbol (max 1-share $ P/L, full history)
 * → system_keltner_scan (stored totals = last ~2y).
 *
 *   npm run analyze-keltner
 *   npm run analyze-keltner -- --symbol AAPL
 *   npm run analyze-keltner -- --limit 10
 */

import { scanKeltner } from "../frontend/src/scanKeltner.js";
import { closePool } from "../backend/src/db.js";
import {
  listActiveSymbols,
  loadBarsForSymbol,
  upsertKeltnerScanRow,
...
async function analyzeOne(sym) {
  ...
  const result = scanKeltner(bars);
  if (!result) {
    return { sym, status: "skip", reason: "analyze failed" };
  }
  await upsertKeltnerScanRow(sym, result);
...
async function main() {
  ...
  if (!PIPELINE_QUIET) {
    console.log(`Keltner optimize ${symbols.length} symbol(s)…\n`);
  }
    ...
    try {
      ...
      if (r.status === "ok") {
        ...
        if (!PIPELINE_QUIET) {
          ...
          console.log(
            `${sym.padEnd(6)} ${r.optEmaPeriod}/${r.optAtrPeriod}x${r.optAtrMult}  RT ${rt}  ${pct}  trades ${r.tradeCount}  ${r.lastSignal}`
          );
        }
      } else {
        skipped++;
