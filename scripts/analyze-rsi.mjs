/**
 * Optimize RSI mean reversion per symbol (max 1-share $ P/L, full history)
 * → system_rsi_scan (stored totals = last ~2y).
 *
 *   npm run analyze-rsi
 *   npm run analyze-rsi -- --symbol AAPL
 *   npm run analyze-rsi -- --limit 10
 */

import { scanRsi } from "../frontend/src/scanRsi.js";
...
async function analyzeOne(sym) {
  ...
  const result = scanRsi(bars);
  if (!result) {
    return { sym, status: "skip", reason: "analyze failed" };
  }
  await upsertRsiScanRow(sym, result);
...
async function main() {
  ...
  if (!PIPELINE_QUIET) {
    console.log(`RSI optimize ${symbols.length} symbol(s)…\n`);
  }
    ...
    try {
      ...
      if (r.status === "ok") {
        ...
        if (!PIPELINE_QUIET) {
          ...
          console.log(
            `${sym.padEnd(6)} ${r.optPeriod}/${r.optOversold}/${r.optOverbought}  RT ${rt}  ${pct}  trades ${r.tradeCount}  ${r.lastSignal}`
          );
        }
      } else {
        skipped++;
