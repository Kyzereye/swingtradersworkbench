/**
 * Optimize Donchian breakout per symbol (max 1-share $ P/L, full history)
 * → system_donchian_scan (stored totals = last ~2y).
 *
 *   npm run analyze-donchian
 *   npm run analyze-donchian -- --symbol AAPL
 *   npm run analyze-donchian -- --limit 10
 */

import { scanDonchian } from "../frontend/src/scanDonchian.js";
import { closePool } from "../backend/src/db.js";
import {
  listActiveSymbols,
  loadBarsForSymbol,
  upsertDonchianScanRow,
...
async function analyzeOne(sym) {
  ...
  const result = scanDonchian(bars);
  if (!result) {
    return { sym, status: "skip", reason: "analyze failed" };
  }
  await upsertDonchianScanRow(sym, result);
...
async function main() {
  ...
  if (!PIPELINE_QUIET) {
    console.log(`Donchian optimize ${symbols.length} symbol(s)…\n`);
  }
    ...
    try {
      ...
      if (r.status === "ok") {
        ...
        if (!PIPELINE_QUIET) {
          ...
          console.log(
            `${sym.padEnd(6)} ${r.optEntryPeriod}/${r.optExitPeriod}  RT ${rt}  ${pct}  trades ${r.tradeCount}  ${r.lastSignal}`
          );
        }
      } else {
        skipped++;
